      ...
      compiler.hooks.thisCompilation.tap(pluginName, function (compilation) {
        compilation.mainTemplate.hooks.requireEnsure.tap(pluginName, function (source, chunk, hash) {
          ...
          if (hasChunkLodaing) {
            ...
            var mediaTypeString = 'var href = "" + chunkId + (mediaType !== "common" ? "."  + mediaType : "") + "." +';
        ...
        cssChunks.forEach(function (chunkName) {
          ...
          Object.keys(splittedValue).forEach(function (mediaType) {
            ...
            if (splitTablet || !/tablet(Portrait|Landscape)/.test(mediaType)) {
              ...
              var splittedMediaChunkName = isCommon ? chunkName : "".concat(chunkId, ".").concat(mediaType, ".").concat(chunkHash, ".css"); // Add chunk to assets
